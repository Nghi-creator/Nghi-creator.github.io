...
import type { AppId, WindowDragHandler, WindowState } from "../../types";
...
export function OSWindow({
  ...
  const windowMode = window.maximized
    ...
    : hasPosition
      ? `relative mt-6 w-full lg:fixed lg:left-[var(--window-x)] lg:top-[var(--window-y)] lg:mt-0 lg:translate-x-0 lg:translate-y-0 ${dimensions[window.id]} lg:min-h-[320px] lg:min-w-[360px] lg:max-h-[calc(100vh-7rem)] lg:max-w-[calc(100vw-2rem)] lg:resize`
      : `relative mt-6 w-full lg:fixed lg:left-1/2 lg:top-1/2 lg:mt-0 lg:-translate-x-1/2 lg:-translate-y-1/2 ${dimensions[window.id]} lg:min-h-[320px] lg:min-w-[360px] lg:max-h-[calc(100vh-7rem)] lg:max-w-[calc(100vw-2rem)] lg:resize`;
  const windowStyle = {
